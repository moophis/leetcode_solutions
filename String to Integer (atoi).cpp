...
public:
    int atoi(const char *str) {
        int ret = 0;
        bool minus = false;  // check whether the number is negative
        bool sign_checked = false; // whether we already know the sign
        
        // first skip all heading blanks
        while (*str == ' ') {
            str++;
        }
        ...
        for (; *str; str++) {
            if (!sign_checked) {
                ...
                if (isdigit(*str)) {
                    int last = ret;
                    ret = ret * 10 + static_cast<int>(*str - '0');
                    if (last != ret / 10) { // integer overflow
